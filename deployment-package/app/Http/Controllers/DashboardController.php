...
class DashboardController extends Controller
{
    ...
    public function exportReport(Request $request)
    {
        ...
        if ($user->hasAdminAccess()) {
            $data = $this->getAdminExportData();
            $title = 'Admin Dashboard Report';
        } elseif ($user->isFleetManager()) {
            $data = $this->getFleetExportData();
            $title = 'Fleet Dashboard Report';
        } elseif ($user->isAccountant()) {
            $data = $this->getFinanceExportData();
            $title = 'Finance Dashboard Report';
        } else {
            $data = $this->getDefaultExportData();
        ...
        switch ($exportType) {
            ...
            case 'html':
                return $this->exportToHTML($data, $title, $printService);
            case 'excel':
                return $this->exportToExcel($data, $title, $printService);
            default:
                return $this->exportToCSV($data, $title, $printService);
        }
    }
