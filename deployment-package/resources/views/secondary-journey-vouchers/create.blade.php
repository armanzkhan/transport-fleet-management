...
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="h3 mb-1">Create Secondary Journey Voucher</h2>
                <p class="text-muted mb-0">Record secondary freight transactions for fuel stations and pumps.</p>
            </div>
            <div>
...
<form id="secondaryJvForm" method="POST" action="{{ route('secondary-journey-vouchers.store') }}">
    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="text-center">
                                ...
                                <h4 class="text-primary" id="totalFreight">₨0.00</h4>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="text-center">
                                <h6 class="text-muted">Total Shortage</h6>
                                <h4 class="text-danger" id="totalShortage">₨0.00</h4>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="text-center">
                                <h6 class="text-muted">Company Deduction</h6>
                                <h4 class="text-warning" id="totalCompanyDeduction">₨0.00</h4>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="text-center">
                                <h6 class="text-muted">Net Amount</h6>
                                <h4 class="text-success" id="netAmount">₨0.00</h4>
                            </div>
                        </div>
                    </div>
                </div>
...
// Go To Navigation Functions for Secondary JV
function goToNavigation(type) {
    ...
    switch(type) {
        case 'date':
            ...
            searchUrl = `{{ route('secondary-journey-vouchers.index') }}?search_date=${currentValue}`;
            break;
        case 'company':
            currentValue = document.getElementById('company').value;
            searchUrl = `{{ route('secondary-journey-vouchers.index') }}?search_company=${encodeURIComponent(currentValue)}`;
            break;
        case 'loading_point':
            // Get first entry's loading point
            const firstLoadingPoint = document.querySelector('.loading-point-select');
            if (firstLoadingPoint) {
                currentValue = firstLoadingPoint.value;
                searchUrl = `{{ route('secondary-journey-vouchers.index') }}?search_loading_point=${encodeURIComponent(currentValue)}`;
            }
            break;
        case 'destination':
            // Get first entry's destination
            const firstDestination = document.querySelector('.destination-select');
            if (firstDestination) {
                currentValue = firstDestination.value;
                searchUrl = `{{ route('secondary-journey-vouchers.index') }}?search_destination=${encodeURIComponent(currentValue)}`;
            }
            break;
    }
...
// Search by Invoice Function for Secondary JV
function searchByInvoice() {
    const invoiceNumber = document.getElementById('invoiceSearch').value.trim();
    
    if (invoiceNumber) {
        const searchUrl = `{{ route('secondary-journey-vouchers.index') }}?search_invoice=${encodeURIComponent(invoiceNumber)}`;
        // Open in new tab to preserve current form
        window.open(searchUrl, '_blank');
    } else {
